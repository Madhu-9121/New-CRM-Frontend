...
const SalesEnquiry = ({ setEditDocument = {}, setSelectedDocument = {}, docType, setrefresh, editDocument = {}, userTokenDetails, navToggleState, selectedInvoiceData, buyerDetails, sellerDetails, handleGoBack, setSelectedBuyer }) => {
  ...
  const [data, setData] = useState(editDocument ? {
    ...
    invCurrency: 'USD', invChargeTitle0: "SUBTOTAL", invChargeTitle1: 'HANDLING', nviChargeTitle2: 'FREIGHT',
    invChargeTitle3: 'MISC.', status: 0, SEDate: new Date().toISOString().split('T')[0],
  ...
  const [preview, setPreview] = useState(editDocument ? { show: editDocument.status === 1 ? false : true } : {})
  const [showPopup, togglePopup] = useState(false)
  const [signdoc, setSigndoc] = useState(false);
  const [SENumber, setSENumber] = useState(editDocument.SENumber || '')
  ...
  const [updateStockItems, setUpdateStockItems] = useState(1)
  ...
  const saveToDb = async (type) => {
    data.SENumber = SENumber
    const objectAPI = {
      docId: editDocument?.SENumber || data.SENumber,
      userId: userId,
      docType: docType || "Sales Enquiry",
      ...
      transaction_timeline: { "Sales Enquiry": new Date().toLocaleString() }

    }
    console.log('objectAPI22', objectAPI)
    if (editDocument) {
      ...
      objectAPI.transaction_timeline = { "Sales Enquiry": new Date().toLocaleString() }
  ...
  function generateSalesQuotationNumber() {
    ...
    // ${year}
    const docNumber = `SE${month}${day}${hours}${minutes}${seconds}`;
  ...
  useEffect(() => {
    ...
    data.SENumber = uniqDocNumber
    if (!SENumber) setSENumber(uniqDocNumber)
  }, [])
  ...
  async function saveCommInvoice() {
    ...
    formData.append("docType", "se")
    formData.append("docTemplate", "Sales Enquiry")
    formData.append("docName", `Sales Enquiry - ${SENumber}`)
    if (data.commInvoice) {
            ...
            {editDocument?.itemStatus === 3 ?
              <button type="button"
                onClick={() => {
                  ...
                  // changes for se to sq
                  // Assign the value of docNumber to SEdocNumber
                  ...
                  // Delete the docNumber key
                  // delete editDocument.docNumber;
                  // delete editDocument.invoiceDate
                  // console.log("logged inside after", editDocument)
                  setSelectedDocument('Sales Quotation')
                }}
                className={` new-btn p-2 text-white`} style={{ width: preview.show ? '200px' : "100%" }}>
                ...
                {preview.show ?
                  <button type="button"
                    onClick={() => {
                      ...
                      data.docId = data.SENumber || SENumber
                      data.docType = "Sales Enquiry"
                      // handleDownloadForContainer(data)
                      // printDiv("invoiceDiv", `Sales Enquiry - ${docNumber}`, [], data, setData, "commInvoice")
                      saveToDb("new")
                      // handleGoBack()
                    }}
        ...
        <div id={"invoiceDiv"} className="p-4"   >
          <div style={{ borderRadius: "5px" }} >
            <div
              ...
              <label className="font-size-18 font-wt-600" >SALES ENQUIRY</label>
            ...
            <div className="row m-0 d-flex " >
              ...
              <div className="col-4 p-0 m-0" style={{ borderRight: '1.5px solid #000', }}>
                <div style={{ borderBottom: '1px solid #000' }} className="d-flex flex-wrap" >
                  <div className="w-50 p-0" >
                    <div className="p-3"
                      style={{ border: '1px solid #000' }}>
                      <label className="font-size-15 font-wt-500" ><u>DOCUMENT NO.</u></label>
                      <div className="position-relative" >
                        <InputForTable isDisabled={preview.show} placeholder={(!data.SENumber && !preview.show) ? "Enter No" : ""} name={"SENumber"} value={SENumber} onChange={handleChange} />
                      </div>
                    </div>
                  </div>
                  <div className="w-50 p-0" >
                    <div className="p-3"
                      ...
                      <div className="position-relative" >
                        <InputForTable isDisabled={preview.show} type={"date"} placeholder={(!data.SEDate && !preview.show) ? "Select Date" : ""} name={"SEDate"} value={data.SEDate} onChange={handleChange} />
                      </div>
                    </div>
                  </div>
                  <div className="w-50 p-0" >
            ...
            <div className="row m-0 d-flex row" style={{ borderRight: "2px solid black", borderBottom: "0px" }} >
