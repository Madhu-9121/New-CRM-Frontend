...
const Financer = ({ userTokenDetails, navToggleState, renderAsChildren, hideGraphs, showForThisUser, changedFilterData, setChangedFilterData }) => {
  ...
  const [servicesPopup, toggleservicespopup] = useState({ show: false, data: [] })
  const [showLoader, setshowLoader] = useState(false)
  const [callHistoryPopup, toggleCallHistoryPopup] = useState({ show: false, data: [] })
  const [includedStatus, setIncludedStatus] = useState([0, 2, 3])
  ...
  const getTasks = (isDownload) => {
    ...
    objectAPI["dateRangeFilter"] = [moment(startDate).format('YYYY-MM-DD'), moment(endDate).format('YYYY-MM-DD')]
    if (isDownload) {
    ...
    call('POST', 'getCRMFinanciers', objectAPI)
      .then((result) => {
        if (isDownload) {
          let finaldata = []
  ...
  const getTasksStats = () => {
    ...
    call('POST', 'getCallListStats', objectAPI).then(result => {
  ...
  async function handleServicesPopup(itemData) {
    // console.log("getTransactionHistoryForInvoiceLimit api resp====>", itemData, apiResp);
    setshowLoader(false)
    toggleservicespopup({ show: true, data: itemData.SERVICES_OFFERED })
  }

  async function handleCallHistoryPopup(itemData) {
    setshowLoader(true)
    let apiResp = await call('POST', 'getFinCallHistory', {
      id: itemData._id
    })
    // console.log("getTransactionHistoryForInvoiceLimit api resp====>", itemData, apiResp);
    setshowLoader(false)
  ...
  function formatDataForTable(data) {
    ...
    data.forEach((item, index) => {
      ...
      row.push(`${item.NAME_TITLE ? item.NAME_TITLE : ''} ${item.CONTACT_PERSON ? item.CONTACT_PERSON : 'NA'} `)
      row.push(item.DESIGNATION)
      row.push(`${item.COUNTRY_CODE ? "+ " + item.COUNTRY_CODE : ''} ${item.CONTACT_NUMBER ? item.CONTACT_NUMBER : 'NA'} `)

      row.push(<ul className='py-0 pl-3 cursor' onClick={() => handleServicesPopup(item)} >
        {item?.SERVICES_OFFERED?.slice(0, 2)?.map(item => {
          return <li >
  ...
  const updateCRMTask = (LOG_TYPE, index, type) => {
    ...
    if (isEmpty(error)) {
      ...
      reqObj["COMPANY_NAME"] = overalldata[selectedExpIndex]?.COMPANY_NAME
      reqObj["id"] = overalldata[selectedExpIndex]?._id

      call('POST', 'updateFinancierTask', reqObj).then(result => {
  ...
  return (
    ...
    <div className={renderAsChildren ? "" : "container-fluid"}>
      ...
      <div className={`modal fade ${servicesPopup.show && "show"}`} style={servicesPopup.show ? { display: "block", "zIndex": '100001' } : {}}>
        <div className="modal-dialog modal-md mr-0 my-0">
          <div className="modal-content submitmodal pb-4"
          >
            ...
            <div className="modal-header border-0">
              <div className="w-100 d-flex align-items-center justify-content-between">
                ...
                <div className="modal-header border-0">
                  <button type="button" className="btn-close" aria-label="Close" onClick={() => toggleservicespopup({ show: false, data: [] })}></button>
                </div>
              </div>
            </div>

            <div className="modal-body px-4">
              {servicesPopup.data.length ? servicesPopup.data.map((item, index) => {
                return (
                  <div className='d-flex flex-row ml-3'>
      ...
      <div className="row">
        {!renderAsChildren ?
          <SideBarV2 state={"taskManagerfinancier"} userTokenDetails={userTokenDetails} /> : null}
            ...
            <>
              {/* <div className='row'>
                <div className='col-md-2'>
                  <label>Date</label>
                  <NewSelect
                    selectData={[{ name: 'Previous Month' }, { name: 'Previous Week' }, { name: 'Yesterday' }, { name: 'Today' }, { name: 'Current Week' }, { name: 'Current Month' }, { name: 'Custom' }]}
                    optionLabel={'name'} optionValue={'name'}
                    onChange={(e) => {
                  ...
                  <div className='card h-75 dashboard-card shadow-sm align-items-center justify-content-center'>
                    <div className='row px-0 w-100'>
                      <div className='w-50 cursor'
                        ...
                        <label className={'font-size-14 font-wt-700 text-color-value cursor'}>{"Financiers"}</label>
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      {LogStatus.map((item, index) => {
                        return <div className='w-20 cursor'
                          onClick={() => {
                            ...
                            temp["Status"]["data"][index]["isChecked"] = true
                            temp["Status"]["data"][5]["isChecked"] = false
                            temp["Status"]["data"][6]["isChecked"] = false
                            for (let i = 0; i <= LogStatus.length - 1; i++) {
                              if (i !== index) {
                                temp["Status"]["data"][i]["isChecked"] = false
                              }
                ...
                <Filter
                  ...
                  <div className="d-flex gap-4">
                    <button className={`new-btn  py-2 px-2 text-white cursor`} onClick={() => {
                      window.location = '/addFinancer'
                    }}>Add new Financier</button>
                  </div>
                </Filter>
              </div>
              ...
              <div className="mb-3">
                <ExpandableTable
                  ...
                  columns={[
                    { name: "Name", filter: false, width: '14%' },
                    { name: "Director Name", filter: false, width: '10%' },
                    { name: "Designation", filter: false, width: '10%' },
                    { name: "Contact Number", filter: false, width: '10%' },
                    { name: "Service", filter: false, width: '12%' },
                    { name: "Admin", filter: false, width: '10%' },
                    { name: "Remark", filter: false, width: '20%' },
                    { name: "Action", width: '5%' }
                  ]}
...
export default connect(mapStateToProps, null)(Financer) 